...
def audio_from_file(file_path, offset=0, duration=0, trim=False,
                    target_sr=16000):
...
class SpectrogramFeatures(nn.Module):
    ...
    __constants__ = ["dither", "preemph", "n_fft", "hop_length", "win_length",
                     "log", "frame_splicing", "window", "normalize", "pad_to",
                     "max_duration", "do_normalize"]
    ...
    def forward(self, x, seq_len):
        ...
        max_len = x.size(-1)
        mask = torch.arange(max_len, dtype=seq_len.dtype).to(
            seq_len.device).expand(x.size(0), max_len) >= seq_len.unsqueeze(1)
...
class FilterbankFeatures(nn.Module):
    ...
    def __init__(self, sample_rate=8000, window_size=0.02, window_stride=0.01,
        ...
        filterbanks = torch.tensor(
            librosa.filters.mel(
                sample_rate, self.n_fft, n_mels=nfilt,
